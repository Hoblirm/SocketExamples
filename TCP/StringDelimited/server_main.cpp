...
int recv_delim(int sockfd, char* buf, size_t len, int flags, char delim){
   int index = 0;
   int status;
   while (index < len) {
     status = recv(sockfd,buf+(index*sizeof(char)),1,flags);
     if (status <= 0) 
     {
       return status;
     }
...
int main()
{
   ...
   do
   {
      ...
      rc = recv_delim(sd2, buffer, sizeof(buffer), 0, '\n');
      if (rc < 0)
      {
         perror("recv() failed");
         break;
